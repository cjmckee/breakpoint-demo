  ...
  return (
    <div className="min-h-screen bg-pixel-bg">
      ...
      <div className="max-w-6xl mx-auto px-4 pb-8">
        ...
        <Card className="mb-6">
          ...
          <div className="border-t-2 border-pixel-border pt-4">
            ...
            {Object.keys(passiveBoosts).length > 0 ? (
              <StatBoostList statBoosts={passiveBoosts} variant="compact" />
            ) : (
        ...
        {/* Item details / swap modal */}
        <Modal
          isOpen={modal.item !== null || modal.showSwapOptions}
          onClose={closeModal}
          size="md"
          title={
            modal.showSwapOptions && modal.slot
              ? player.equippedItems[modal.slot]
                ? `Swap ${SLOT_NAMES[modal.slot]}`
                : `Equip ${SLOT_NAMES[modal.slot]}`
              : modal.item?.name ?? ''
          }
        >
          {modal.showSwapOptions && modal.slot ? (
